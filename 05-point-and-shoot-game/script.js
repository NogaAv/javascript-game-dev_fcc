...
class Raven{
    constructor(){
        ...
        this.hasTrail = Math.random() > 0.5; //true for roughly 50% of ravens. I want half the ravens to have particles trail, not all(expensive)
    }
    update(deltatime){
        ...
        if(this.timeSliceFlap > this.flapInterval){
            ...
            this.timeSliceFlap = 0;
            if(this.hasTrail){
                for(let i = 0 ; i < 5 ; ++i){ //be carefull to not allow to many trails since it will affect performance and frame speed
                    particles.push(new Particle(this.x, this.y, this.width, this.color));//we use the hit-box color also for coloring the raven's wings flappings particles
                }
            }
        }
...
//3:41 - adding particles (circles traces when flipping wings)
let particles = [];
class Particle{
    constructor(x, y, size, color){
        this.size = size;
        this.x = x + this.size/2 + Math.random() * 50 - 25;//'this.size/2' = I want the trailing to start after the raven. 'Math.random() * 50 - 25' = I want to randomize the x and y coordinates of the trailing(more smoke realistic)
        this.y = y + this.size/3 + Math.random() * 50 - 25;
        this.radius = Math.random() * this.size/10; //I want radius of particles to correspond to the raven size
        this.maxRadius = Math.random() * 20 + 35;
        this.markedForDeletion = false;
        this.speedX = Math.random() *1 + 0.5; //the particles will move to right
        this.color = color;
    }
    update(){
        this.x += this.speedX;
        this.radius += 0.5; //it will increase radius in 0.2 per frame
        if(this.radius > this.maxRadius - 5) this.markedForDeletion = true;//why 'this.maxRadius - 5'? because I want the particles to be marked for deletion abit sooner- so that the opacity will be correct (without the -5 the last circle of particle has opacity=1 i.e- fully seen)
    }
    draw(){ //will draw a simple circle
        ctx.save();//We added save() + restore()- because the ravens also got affected by the 'ctx.globalAlpha' call and are flickering. to avoid that the save() saves the state of canvas and after the drawing of particles we restore it(to not leave affect on other objects drawned). save()+restore() mostly used when rotating objets on screen
        ctx.globalAlpha = 1 - this.radius/this.maxRadius; //I want the trailing particles to fade(opacity increase)
        ctx.beginPath(); //to start the drawing
        ctx.fillStyle = this.color;         
        ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2); //arc() - to draw a circle: arc(x, y, radius, start angle, end angle)
        ctx.fill();
        ctx.restore();
    }
}
...
function animate(timestamp){
    ...
    [...particles, ...ravens, ...explosions].forEach(object => object.draw());//I need particles to be added first to array, so that the trails will be drawned behind the ravens
